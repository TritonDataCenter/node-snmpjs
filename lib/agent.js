...
Agent.prototype._transmit_response = function _transmit_response(sock, rsp) {
	...
	sock.send(rsp.raw.buf, 0, rsp.raw.len, dst.port, dst.address,
		function (err, len) {
			if (err)
				self._log.warn({raw: rsp.raw, dst: dst, snmpmsg: rsp, err: err}, 'Response error')
			if (local_sock)
				sock.close();
		});
};
...
Agent.prototype._process_req = function _process_req(req) {
	...
	if (this._community && this._community !== req.community.toString()) {
		this._log.warn({ snmpmsg: req },
				'Community string provided does not match Agent\'s community string');
		return;
	}
