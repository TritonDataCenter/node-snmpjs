...
Agent.prototype._dispatch_varbind = function (req, rsp, idx, next) {
	...
	if (!loc) {
		if (req.pdu.op == PDU.GetRequest ||
			...
			var rsd = data.createSnmpData({ type: 'Null',
			    value: data.noSuchObject });
			var rsvb = varbind.createSnmpVarbind({ oid: rqvb.oid,
			    data: rsd });
			next(rsvb);
			return;
		}
...
Agent.prototype._process_simple = function (req, rsp) {
	var self = this;
	var pdu = rsp.pdu;
...
Agent.prototype._process_next = function (req, rsp) {
	/* XXX */
};
...
Agent.prototype._process_req = function (req) {
	...
	case PDU.GetRequest:
	case PDU.SetRequest:
		this._process_simple(req, rsp);
		break;
	case PDU.GetNextRequest:
		this._process_next(req, rsp);
		break;
