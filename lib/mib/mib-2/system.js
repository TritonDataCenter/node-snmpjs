...
{
	/* XXX configuration */
	var val = snmp.createSnmpData({ type: 'OctetString',
	    value: 'node-snmpjs' });
...
mib2_system_sysObjectID(arg)
{
	...
	var val = snmp.createSnmpData(data);
	snmp.util.readOnlyScalar(arg, val);
}
...
{
	/* XXX introspection */
	var val = snmp.createSnmpData({ type: 'TimeTicks', value: 0 });
	snmp.util.readOnlyScalar(arg, val);
}
...
mib2_system_sysName(arg)
{
	...
	var val = snmp.data.createSnmpData({ type: 'OctetString',
	    value: nodename });
	snmp.util.readOnlyScalar(arg, val);
}
...
{
	/* XXX configuration */
	var val = snmp.createSnmpData({ type: 'OctetString', value: 'home' });
	snmp.util.readOnlyScalar(arg, val);
}
...
{
	/* XXX configuration */
	var val = snmp.createSnmpData({ type: 'Integer', value: 72 });
	snmp.util.readOnlyScalar(arg, val);
}
