...
SnmpVarbind(arg)
{
	...
	this.__defineSetter__('data', function (v) {
		if (typeof (v) === 'object' && v instanceof data.SnmpData)
			self._data = v;
		else if (typeof (v) === 'object' && v instanceof ASN1.Reader)
			self._data = data.createSnmpData({ value: v });
		else if (typeof (v) === 'object')
			self._data = data.createSnmpData(v);
	});
...
SnmpVarbind.prototype.clone = function () {
	var oclone, dclone;
...
SnmpVarbind.prototype.encode = function (writer) {
...
module.exports = {
	...
	createSnmpVarbind: createSnmpVarbind
};
