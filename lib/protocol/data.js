...
var TypeConflictError = require('../errors/varbind.js').TypeConflictError;
 ...
 * @return {String} the canonical form of oid.
...
SnmpData.prototype.clone = function () {
	return (new this.constructor(this._value));
};

SnmpData.prototype.encode = function (writer) {
...
SnmpInteger.prototype.encode = function (writer) {
...
SnmpOctetString.prototype.encode = function (writer) {
...
SnmpOID.prototype.encode = function (writer) {
...
SnmpIpAddress.prototype.encode = function (writer) {
...
SnmpCounter32.prototype.encode = function (writer) {
	var bytes = [];
	var v = this._value;
...
SnmpCounter64.prototype.encode = function (writer) {
	var bytes = [];
	var v = {
		hi: this._value.hi,
		lo: this._value.lo
...
SnmpNull.prototype.encodeValue = function (writer) {
	if (this._value === null) {
		writer.writeNull();
	} else {
...
init()
{
	...
	for (i = 0; i < stock.length; i++) {
		registerSnmpType({
			type: stock[i].prototype._typename,
			tag: stock[i].prototype._tag,
			f: stock[i]
		});
	}
	...
	registerSnmpType({
		type: 'Null',
		tag: ASN1.Context | varbind.noSuchObject
	});
	registerSnmpType({
		type: 'Null',
		tag: ASN1.Context | varbind.noSuchInstance
	});
	registerSnmpType({
		type: 'Null',
		tag: ASN1.Context | varbind.endOfMibView
	});
...
module.exports = {
	SnmpData: init(),
	createSnmpData: createSnmpData,
	registerSnmpType: registerSnmpType,
	isSnmpTagRegistered: isSnmpTagRegistered,
	isSnmpTypeRegistered: isSnmpTypeRegistered,
